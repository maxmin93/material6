...
(function( agens, undefined ) {
  ...
  agens.graph.ready = function(cyGraph){
    ...
    cyGraph.$api.panzoom = cyGraph.panzoom({
    ...
    // mouse wheel disable
    cyGraph.$api.panzoom.userZoomingEnabled( false );
    ...
    cyGraph.on('tap', function(e){
      ...
      if( e.target === cyGraph ){
        ...
        if( cyGraph.$api.view !== undefined ) cyGraph.$api.view.removeHighlights();
    ...
    cyGraph.$api.edge = cyGraph.edgehandles({
        preview: true,
        hoverDelay: 150,
        handleNodes: 'node',
    ...
    cyGraph.$api.edge.disable();

    cyGraph.$api.undo = cyGraph.undoRedo({
    ...
    cyGraph.$api.view = cyGraph.viewUtilities({
    ...
    cyGraph.$api.findNeighbors = function( node, uniqLabels, maxHops, callback=undefined ){
      // empty collection
      var connectedNodes = cyGraph.collection();
      ...
      connectedNodes.difference(node).forEach(elem => {
        var collection = cyGraph.$api.view.findNeighbors(elem, uniqLabels.slice(0), maxHops);
      ...
      cyGraph.cxtmenu({
        ...
        commands: [{
            ...
            select: function(){
              ...
              cyGraph.$api.view.removeHighlights();
              selected.unselect();
              unselected.select();
            }
          },{
            ...
            select: function(){
              cyGraph.$api.view.hide(cyGraph.elements(":unselected"));
            },
          },{
            content: '<span style="display:inline-block; width:20px; font-size:10pt">Show all</span>',
            select: function(){
              cyGraph.$api.view.show(cyGraph.elements(":hidden"));
            },
          },{
